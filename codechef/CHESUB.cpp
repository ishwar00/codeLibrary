...
public:
    ...
    void solve() {
        ...
        cache = vector<vector<int64_t>>(N + 1, vector<int64_t>(K + 1));
        best_k = vector<int64_t>(N + 1);

        for (int64_t i { 1 }, mx { 0 }; i <= N; ++i) {
            mx += A[i];
            cache[i][1] = max(cache[i - 1][1], mx);
            best_k[i] = cache[i][1];
            if (mx < 0) mx = 0;
        }

        for (int64_t j { 2 }; j <= K; ++j) {
            vector<int64_t> buffer(N + 1);
            for (int64_t i { 1 }; i <= N; ++i) {
                if (i >= j) {
                    cache[i][j] = max(cache[i - 1][j], best_k[i - 1]) + A[i] * j;
                } else {
                    cache[i][j] = INT_MIN;
                }
                buffer[i] = max(cache[i][j], buffer[i]);
            }
            best_k = std::move(buffer);
        }
        cout << *(max_element(best_k.begin(), best_k.end())) << nl;
